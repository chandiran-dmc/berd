...
export class ConnectionCenterHandleOverlayUtil extends OverlayUtil<TLConnectionCenterHandleOverlay> {
  ...
  override getGeometry(overlay: TLConnectionCenterHandleOverlay): Geometry2d {
    const radius =
      CONNECTION_CENTER_HANDLE_HOVER_SIZE_PX / 2 / this.editor.getZoomLevel();
    return new Circle2d({
  ...
  override onPointerDown(
    overlay: TLConnectionCenterHandleOverlay,
    _info: TLPointerEventInfo,
  ) {
    const connection = this.editor.getShape<ConnectionShape>(
      overlay.props.shapeId,
    );
  ...
  override render(
    ...
    // Sizes are specified in screen pixels, so divide by zoom to get page units.
