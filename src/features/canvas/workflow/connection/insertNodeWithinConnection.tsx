import { type Box, createShapeId, type Editor, type TLShapeId } from "tldraw";
...
export function insertNodeWithinConnection(
  ...
  onCanvasComponentPickerState.set(editor, {
    ...
    onPick: (nodeType) => {
      ...
      const startBounds = editor.getShapePageBounds(
      ...
      const newNodeIdealX =
        (startBounds.right + endBounds.left - NODE_WIDTH_PX) / 2;
      ...
      const firstOutputPort = Object.values(ports).find(
        (p) => p.terminal === "start",
      );
...
function moveNodesIfNeeded(
  ...
  const toNudgeRight = new Map<
    TLShapeId,
    { initialX: number; amount: number }
  >();
  ...
  function visit(nodeId: TLShapeId, parentExpandedBounds: Box) {
    ...
    const currentNudge = toNudgeRight.get(nodeId) ?? {
      initialX: node.x,
      amount: 0,
    };
    ...
    nodeBounds
      .translate({ x: newNudgeAmount, y: 0 })
