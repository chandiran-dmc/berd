 ...
 */
import { createComputedCache, type Editor, type TLShapeId } from "tldraw";
...
export function getNodePorts(editor: Editor, shape: NodeShape | TLShapeId) {
  return (
    nodePortsCache.get(editor, typeof shape === "string" ? shape : shape.id) ??
    {}
  );
}
  ...
  (editor: Editor, node: NodeShape) => {
    ...
    for (const binding of bindings) {
      const oppositeTerminal =
        binding.props.terminal === "start" ? "end" : "start";
      const oppositeBinding = getConnectionBindings(editor, binding.fromId)[
        oppositeTerminal
      ];
...
const nodeInputPortValuesCache = createComputedCache(
  ...
  (editor: Editor, node: NodeShape) => {
    ...
    for (const connection of connections) {
      ...
      const connectedShapeOutputs = getNodeOutputPortInfo(
        editor,
 ...
 */
export function getNodeOutputPortInfo(
  editor: Editor,
  shape: NodeShape | TLShapeId,
): InfoValues {
  return (
    nodeOutputPortInfoCache.get(
      editor,
      typeof shape === "string" ? shape : shape.id,
    ) ?? {}
  );
...
export function getAllConnectedNodes(
  ...
  const toVisit = [
    typeof startingNode === "string" ? startingNode : startingNode.id,
  ];
