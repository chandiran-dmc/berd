...
export class AddNodeDefinition extends NodeDefinition<AddNode> {
  ...
  onPortDisconnect(shape: NodeShape, _node: AddNode, _portId: string): void {
    updateNode<AddNode>(this.editor, shape, (node) => ({
      ...node,
      items: ensureFinalEmptyItem(this.editor, shape, node.items, {
        removeUnused: true,
      }),
    }));
  }
        ...
        <NodeInputRow
          ...
          onChange={(newValue) => {
            updateNode<AddNode>(editor, shape, (node) => ({
              ...node,
              items: ensureFinalEmptyItem(editor, shape, {
                ...node.items,
                [idx]: newValue,
              }),
            }));
          ...
          onBlur={() => {
            updateNode<AddNode>(editor, shape, (node) => ({
              ...node,
              items: ensureFinalEmptyItem(editor, shape, node.items, {
                removeUnused: true,
              }),
            }));
          }}
