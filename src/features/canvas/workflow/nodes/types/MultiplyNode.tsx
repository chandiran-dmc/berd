...
export class MultiplyNodeDefinition extends NodeDefinition<MultiplyNode> {
  ...
  async execute(
    ...
    const result =
      (inputs.multiplicand ?? node.a) * (inputs.multiplier ?? node.b);
...
export function MultiplyNodeComponent({
  shape,
  node,
}: NodeComponentProps<MultiplyNode>) {
  ...
  return (
    <>
      <NodeInputRow
        ...
        value={node.a}
        onChange={(newValue) =>
          updateNode<MultiplyNode>(editor, shape, (node) => ({
            ...node,
            a: newValue,
          }))
        }
