...
export class ExecutionGraph {
   ...
   */
  private readonly nodesById = new AtomMap<TLShapeId, ExecutionGraphNode>(
    "node by id",
  );
  ...
  private async executeNodeIfReady(nodeId: TLShapeId) {
    ...
    for (const connection of node.connections) {
      ...
      if (dependency) {
        ...
        const outputs = getNodeOutputPortInfo(
          this.editor,
    ...
    for (const connection of Object.values(node.connections)) {
      ...
      executingDependentPromises.push(
        this.executeNodeIfReady(connection.connectedShapeId),
      );
    }
