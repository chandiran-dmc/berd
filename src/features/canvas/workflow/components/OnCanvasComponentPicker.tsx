...
import { getNodeDefinitions, type NodeType } from "../nodes/nodeTypes";
import type { NodeDefinition } from "../nodes/types/shared";
...
export const onCanvasComponentPickerState =
  new EditorAtom<OnCanvasComponentPickerState | null>(
    ...
    <OnCanvasComponentPickerDialog onClose={onClose}>
      <TldrawUiMenuGroup id="math">
        <OnCanvasComponentPickerItem
          definition={nodeDefs.add}
          onClose={onClose}
        />
        <OnCanvasComponentPickerItem
          definition={nodeDefs.subtract}
          onClose={onClose}
        />
        <OnCanvasComponentPickerItem
          definition={nodeDefs.multiply}
          onClose={onClose}
        />
        <OnCanvasComponentPickerItem
          definition={nodeDefs.divide}
          onClose={onClose}
        />
      </TldrawUiMenuGroup>
      <TldrawUiMenuGroup id="logic">
        <OnCanvasComponentPickerItem
          definition={nodeDefs.conditional}
          onClose={onClose}
        />
      </TldrawUiMenuGroup>
...
function OnCanvasComponentPickerDialog({
  ...
  const editor = useEditor();
  const location = useValue(
    "location",
    () => onCanvasComponentPickerState.get(editor)?.location,
    [editor],
  );
