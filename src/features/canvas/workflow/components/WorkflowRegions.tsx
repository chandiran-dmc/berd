...
function findWorkflowRegions(editor: Editor): WorkflowRegion[] {
  ...
  function visit(node: NodeShape, currentWorkflow?: Set<TLShapeId>) {
    ...
    for (const connection of getNodePortConnections(editor, node)) {
      visit(
        editor.getShape(connection.connectedShapeId) as NodeShape,
        currentWorkflow,
      );
    }
  }
  ...
  return Array.from(
    new Set(workflowSetsByShape.values()),
    (nodeIds): WorkflowRegion | null => {
      ...
      for (const nodeId of nodeIds) {
        ...
        const hasInputs = getNodePortConnections(editor, nodeId).some(
          (c) => c.terminal === "end",
        );
        if (!hasInputs) {
  ...
  ).filter((workflow): workflow is WorkflowRegion =>
    Boolean(workflow && workflow.nodes.size > 1),
  ); // Only show workflows with multiple nodes
}
...
export function WorkflowRegions() {
  ...
  const workflows = useValue("workflows", () => findWorkflowRegions(editor), [
    editor,
  ]);

  return workflows.map((workflow) => (
    <WorkflowRegion
      key={[...workflow.nodes].sort().join(":")}
      workflow={workflow}
    />
  ));
}
...
function WorkflowRegion({ workflow }: { workflow: WorkflowRegion }) {
  ...
  const isExecuting = useValue("isExecuting", () => {
  ...
  useQuickReactor("WorkflowRegion positioning", () => {
    ...
    const buttonNeedsTop = position.x < 70;
    ref.current.style.setProperty(
      "--workflow-button-left",
      buttonNeedsTop ? "70px" : "-60px",
    );
    ref.current.style.setProperty(
      "--workflow-button-top",
      buttonNeedsTop ? (position.y < 70 ? "8px" : "-60px") : "0px",
    );
  ...
  return (
    <div
      className={classNames("WorkflowRegion", {
        WorkflowRegion_executing: isExecuting,
      })}
      ref={ref}
    >
      <button
