...
export class PointingPort extends StateNode {
  ...
  override onPointerMove(info: TLPointerEventInfo): void {
    ...
    if (this.editor.inputs.getIsDragging()) {
      const portInfo = this.info;
      if (!portInfo) return;
      const allowsMultipleConnections = portInfo.terminal === "start";
      const hasExistingConnection = getNodePortConnections(
        this.editor,
        portInfo.shapeId,
      ).find((c) => c.ownPortId === portInfo.portId);
      ...
      if (!allowsMultipleConnections && hasExistingConnection) {
        const shape = this.editor.getShape(hasExistingConnection.connectionId);
        const handle = this.editor
          .getShapeHandles(hasExistingConnection.connectionId)
          ?.find((candidate) => candidate.id === portInfo.terminal);
      ...
      const connectingTerminal = portInfo.terminal;
      ...
      const shape = this.editor.getShape(connectionShapeId);
      if (!shape || !handle) {
  ...
  private onClick() {
    ...
    if (portInfo.terminal !== "start") return;
    ...
    const bounds = this.editor.getShapePageBounds(portInfo.shapeId);
    if (!bounds) return;
    ...
    const targetPositionInConnectionSpace = this.editor
      ...
      .addXY(
        0,
        NODE_HEADER_HEIGHT_PX + NODE_ROW_HEADER_GAP_PX + NODE_ROW_HEIGHT_PX / 2,
      );
    ...
    onCanvasComponentPickerState.set(this.editor, {
      ...
      onPick: (nodeType, terminalInPageSpace) => {
        ...
        const firstInputPort = Object.values(ports).find(
          (p) => p.terminal === "end",
        );
        if (firstInputPort) {
          this.editor.updateShape({
      ...
      onClose: () => {
        ...
        if (!connection || !this.editor.isShapeOfType(connection, "connection"))
          return;
