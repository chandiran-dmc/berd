...
import { type TLShapeId, useEditor, useValue, type VecModel } from "tldraw";
...
export function Port({
  ...
  const isHinting = useValue("isHinting", () => {
    const { hintingPort } = portState.get(editor);
    return (
      hintingPort &&
      hintingPort.portId === portId &&
      hintingPort.shapeId === shapeId
    );
  ...
  const isEligible = useValue("isEligible", () => {
